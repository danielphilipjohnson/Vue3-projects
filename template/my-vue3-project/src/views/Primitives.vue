  ...
  <div class="about">
    ...
    <div class="p-6 border">
      <ElButton
        ...
        @clicked="onButtonClick('first')"
      />
      <ElButton
        :disabled="true"
        label="This is a disabled button"
        addCss="ml-2"
        @clicked="onButtonClick('second')"
      />
    </div>
    ...
    <div class="p-6 border">
      <ElToggle
        :checked="
          toggleButtonsState.state.find((item) => item.key === 'a')?.checked
        "
        :disabled="false"
      ...
      <ElToggle
        :checked="
          toggleButtonsState.state.find((item) => item.key === 'b')?.checked
        "
        :disabled="true"
        addCss="ml-2"
      ...
      <ElToggle
        :checked="
          toggleButtonsState.state.find((item) => item.key === 'c')?.checked
        "
        :disabled="false"
        addCss="ml-2"
...
export default defineComponent({
  ...
  setup() {
    ...
    const onToggleClicked = (key: string) => {
      ...
      const stateItem = toggleButtonsState.state.find(
        (item) => item.key === key
      );
      if (stateItem) {
